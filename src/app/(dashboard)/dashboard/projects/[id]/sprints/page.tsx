...
import {
  ...
  Clock, PlayCircle, X, Loader2, ArrowLeft, ListChecks, Flag,
  Users, TrendingUp, Flame, AlertCircle, Play, CheckCircle, Lock,
...
const STATUS_CONFIG = {
  planning:   { label: "Planning",   icon: Clock,         color: "slate",  bg: "bg-slate-50",   border: "border-slate-200",  badge: "bg-slate-100 text-slate-700 border-slate-200", leftAccent: "bg-slate-400" },
  active:     { label: "Active",     icon: PlayCircle,    color: "indigo", bg: "bg-indigo-50",  border: "border-indigo-200", badge: "bg-indigo-100 text-indigo-700 border-indigo-200", leftAccent: "bg-indigo-500" },
  completed:  { label: "Completed",  icon: CheckCircle2,  color: "emerald",bg: "bg-emerald-50", border: "border-emerald-200",badge: "bg-emerald-100 text-emerald-700 border-emerald-200",leftAccent: "bg-emerald-500" },
};
...
function getSprintDaysInfo(sprint: Sprint): { daysLeft: number; daysElapsed: number; totalDays: number; percentElapsed: number } {
  const now = Date.now();
  ...
  const percentElapsed = totalDays > 0 ? Math.round((daysElapsed / totalDays) * 100) : 0;
...
export default function SprintsPage() {
  ...
  const [projectAssignments, setProjectAssignments] = useState<{ task_name: string; resource_id: string }[]>([]);
  ...
  const [form, setForm] = useState({ name: "", goal: "", start_date: "", end_date: "", selectedMilestoneTitle: "" });
  const [deletingSprintId, setDeletingSprintId] = useState<string | null>(null);
  const [restoringSprintId, setRestoringSprintId] = useState<string | null>(null);
  ...
  const sprintsByStatus = useMemo(() => {
    const active = sprints.filter(s => s.status === 'active').sort((a, b) => new Date(b.end_date).getTime() - new Date(a.end_date).getTime());
    const planning = sprints.filter(s => s.status === 'planning').sort((a, b) => new Date(a.start_date).getTime() - new Date(b.start_date).getTime());
    const completed = sprints.filter(s => s.status === 'completed').sort((a, b) => new Date(b.end_date).getTime() - new Date(a.end_date).getTime());
  ...
  const stats = useMemo(() => {
    ...
    const completedTasks = sprints.reduce((sum, s) => sum + (s.done_count || 0), 0);
    const velocity = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0;
    return { total, activeSprints, planningSprints, completedSprints, totalTasks, completedTasks, velocity };
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [{ data: sprintRows }, { data: deletedSprintRows }] = await Promise.all([
        supabase
          .from("sprints")
          .select("*")
      ...
      if (sprintRows && sprintRows.length > 0) {
        const memberRows: Array<{ id: string; full_name: string; job_title: string }> =
          membersRes.members || [];
        const [{ data: taskRows }] = await Promise.all([
          supabase.from("sprint_tasks").select("sprint_id, status, assigned_to").eq("project_id", projectId),
        ]);

        const enriched = sprintRows.map((s: any) => {
          const st = (taskRows || []).filter((t: any) => t.sprint_id === s.id);
          const assigneeIds = [...new Set(st.map((t: any) => t.assigned_to).filter(Boolean))];
          const assignees = assigneeIds
            .map((id: string) => (memberRows || []).find((m: any) => m.id === id))
            .filter(Boolean) as Array<{ id: string; full_name: string; job_title: string }>;
          return {
            ...s,
  ...
  const handleCreate = async () => {
    ...
    setCreating(true);
    setError("");
    const milestone_ids = form.selectedMilestoneTitle ? [form.selectedMilestoneTitle] : [];
    const res = await fetch("/api/sprints/create", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ projectId, workspaceId, name: form.name, goal: form.goal, start_date: form.start_date, end_date: form.end_date, milestone_ids }),
    });
    ...
    const newSprint = { ...data.sprint, task_count: 0, done_count: 0, team_count: 0 };
    setSprints((prev) =>
      [...prev, newSprint].sort(
    ...
    setShowModal(false);
    setForm({ name: "", goal: "", start_date: "", end_date: "", selectedMilestoneTitle: "" });
    setCreating(false);
  };
  ...
  const handleDeleteSprint = async (sprintId: string, sprintName: string) => {
    if (!confirm(`Move "${sprintName}" to trash? It can be restored within 30 days.`)) return;
    setDeletingSprintId(sprintId);
    setSprintActionError("");
    const res = await fetch(`/api/sprints/manage?id=${sprintId}`, { method: "DELETE" });
    const data = await res.json();
    if (!res.ok) {
    ...
    if (deleted) {
      setDeletedSprints(prev => [{ id: deleted.id, name: deleted.name, deleted_at: new Date().toISOString() }, ...prev]);
    }
    setDeletingSprintId(null);
  };
  ...
  const handleRestoreSprint = async (sprintId: string) => {
    ...
    setSprintActionError("");
    const res = await fetch(`/api/sprints/manage?id=${sprintId}`, { method: "PATCH" });
    const data = await res.json();
    if (!res.ok) {
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-8">
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage your sprint lifecycle and track team progress</p>
        </div>
        {isAdmin && (
          <button
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Total Sprints</p>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Active Now</p>
              <Flame size={16} className="text-orange-500" />
            </div>
            <p className="text-3xl font-bold text-slate-900">{stats.activeSprints}</p>
          </div>

          <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Planned</p>
              <Clock size={16} className="text-slate-500" />
            </div>
            <p className="text-3xl font-bold text-slate-900">{stats.planningSprints}</p>
          </div>

          <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Velocity</p>
              <TrendingUp size={16} className="text-emerald-500" />
            </div>
            <p className="text-3xl font-bold text-slate-900">{stats.velocity}%</p>
          </div>
        </div>
      )}
      ...
      {sprints.length === 0 ? (
        <div className="bg-gradient-to-br from-indigo-50 to-violet-50 border-2 border-dashed border-indigo-200 rounded-3xl p-20 text-center">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mb-2">No Sprints Yet</h2>
          <p className="text-slate-600 mb-8 max-w-md mx-auto">
            Create your first sprint to start tracking team progress and delivering value in organized iterations.
          </p>
          {isAdmin && (
            <button
              onClick={handleOpenModal}
          ...
          {sprintsByStatus.active.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-5 pb-3 border-b border-slate-200">
                ...
                <h2 className="text-lg font-bold text-slate-900">Active Sprints</h2>
              ...
              <div className="grid gap-4">
                {sprintsByStatus.active.map((sprint) => {
                  ...
                  const linkedMilestoneItems = (sprint.milestone_ids || []).length > 0
                    ? (sprint.milestone_ids || []).map((title, i) => {
                        const idx = milestones.findIndex(m => m.title === title);
                        const ms = milestones[idx >= 0 ? idx : i];
                        return ms ? { title: ms.title, weekNum: ms.week || ms.week_number || idx + 1 } : null;
                      }).filter(Boolean) as { title: string; weekNum: number }[]
                    : (projectStart > 0 ? getMilestonesForSprint(sprint, milestones, projectStart) : []);
                  const daysInfo = getSprintDaysInfo(sprint);
                  const cfg = STATUS_CONFIG[sprint.status];

                  return (
                    <div key={sprint.id} className={`bg-white border ${cfg.border} rounded-2xl overflow-hidden hover:shadow-lg transition-all group`}>
                      ...
                      <div className="p-6">
                        <div className="flex items-start justify-between gap-4 mb-4">
                          <div className="flex-1 min-w-0">
                            ...
                            <h3 className="text-lg font-bold text-slate-900 leading-tight">{sprint.name}</h3>
                            {sprint.goal && <p className="text-sm text-slate-600 mt-1">{sprint.goal}</p>}
                          </div>
                          <Link
                            href={`/dashboard/projects/${projectId}/sprints/${sprint.id}`}
                            className="flex-shrink-0 px-3 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-lg font-semibold text-xs flex items-center gap-1.5 transition-colors border border-indigo-100"
                          >
                        ...
                        <div className="mb-4">
                          <div className="flex justify-between text-[11px] font-semibold text-slate-600 mb-1.5">
                            <span>{daysInfo.daysElapsed}/{daysInfo.totalDays} days • Day {daysInfo.daysElapsed}</span>
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-4">
                          <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                            <p className="text-[10px] font-bold text-slate-500 uppercase mb-1">Tasks</p>
                            <p className="text-xl font-bold text-slate-900">{sprint.done_count}/{sprint.task_count}</p>
                          ...
                          <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                            <p className="text-[10px] font-bold text-slate-500 uppercase mb-1">Progress</p>
                            <p className="text-xl font-bold text-slate-900">{progress}%</p>
                          </div>
                          <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                            <p className="text-[10px] font-bold text-slate-500 uppercase mb-1">Team</p>
                            <p className="text-xl font-bold text-slate-900">{sprint.team_count || 0}</p>
                          </div>
                        </div>
                        ...
                        {sprint.assignees && sprint.assignees.length > 0 && (
                          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-slate-100">
                            ...
                            <span className="text-xs text-slate-500 font-medium">
                              {sprint.assignees.length} member{sprint.assignees.length !== 1 ? "s" : ""} assigned
                            </span>
                          </div>
                        )}
          ...
          {sprintsByStatus.planning.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-5 pb-3 border-b border-slate-200">
                ...
                <h2 className="text-lg font-bold text-slate-900">Upcoming Sprints</h2>
              ...
              <div className="grid gap-4">
                {sprintsByStatus.planning.map((sprint) => {
                  ...
                  const linkedMilestoneItems = (sprint.milestone_ids || []).length > 0
                    ...
                    : (projectStart > 0 ? getMilestonesForSprint(sprint, milestones, projectStart) : []);
                  const daysUntilStart = Math.ceil((new Date(sprint.start_date).getTime() - Date.now()) / DAY);
                  const cfg = STATUS_CONFIG[sprint.status];

                  return (
                    <div key={sprint.id} className={`bg-white border ${cfg.border} rounded-2xl overflow-hidden hover:shadow-lg transition-all`}>
                      <div className={`h-1 ${cfg.leftAccent}`} />
                      <div className="p-6">
                        <div className="flex items-start justify-between gap-4 mb-4">
                          <div className="flex-1 min-w-0">
                        ...
                        <div className="flex gap-3 mb-4 text-sm">
                          <span className="text-slate-600 font-medium">{sprint.task_count || 0} tasks planned</span>
                          {sprint.team_count ? (
                            <span className="text-slate-600 font-medium flex items-center gap-1">
                              <Users size={14} /> {sprint.team_count} assigned
                            </span>
                          ) : (
                            <span className="text-slate-400 font-medium">No assignments yet</span>
                          )}
                        </div>
          ...
          {sprintsByStatus.completed.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-5 pb-3 border-b border-slate-200">
                ...
                <h2 className="text-lg font-bold text-slate-900">Completed Sprints</h2>
              ...
              <div className="grid gap-4">
                {sprintsByStatus.completed.map((sprint) => {
                  ...
                  return (
                    <div key={sprint.id} className={`bg-white border ${cfg.border} rounded-2xl overflow-hidden opacity-75 hover:opacity-100 transition-opacity`}>
                      <div className={`h-1 ${cfg.leftAccent}`} />
                      <div className="p-6">
                        <div className="flex items-start justify-between gap-4 mb-4">
                          <div className="flex-1 min-w-0">
                          ...
                          <Link
                            href={`/dashboard/projects/${projectId}/sprints/${sprint.id}`}
                            className="flex-shrink-0 px-3 py-2 bg-slate-50 text-slate-700 hover:bg-slate-100 rounded-lg font-semibold text-xs flex items-center gap-1.5 transition-colors border border-slate-200"
                          >
      ...
      {sprintActionError && (
        <div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-xl">
          <AlertCircle size={15} />
          {sprintActionError}
          <button onClick={() => setSprintActionError("")} className="ml-auto text-red-400 hover:text-red-600">
            <X size={14} />
          </button>
        </div>
      )}
                  ...
                  <div key={s.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-slate-50/60 transition-colors">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400 mt-0.5">
                        {days > 0 ? `Permanently deleted in ${days} day${days !== 1 ? "s" : ""}` : "Expires today"}
                      </p>
                    </div>
                    <span
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-900">Create New Sprint</h2>
              <button
            ...
            <div className="space-y-4">
              ...
              {milestones.length > 0 && (
                <div>
                  <label className="text-sm font-semibold text-slate-700 flex items-center gap-1.5 mb-1.5">
                    <Flag size={13} className="text-violet-500" /> Link to Milestone <span className="text-slate-400 font-normal text-xs">(optional)</span>
                  </label>
                  <select
                    value={form.selectedMilestoneTitle}
                    onChange={e => setForm(p => ({ ...p, selectedMilestoneTitle: e.target.value }))}
                    className="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400"
                  >
                    ...
                    {milestones.map((ms, idx) => {
                      ...
                      return (
                        <option key={idx} value={ms.title} disabled={isCompleted}>
                          {isCompleted ? "✓ " : ""}Week {ms.week || ms.week_number || idx + 1}: {ms.title}{isCompleted ? " (Completed)" : ""}
                        </option>
                      );
                    })}
                  </select>
                  {form.selectedMilestoneTitle && milestones.find(m => m.title === form.selectedMilestoneTitle)?.status === "completed" && (
                    <p className="text-xs text-red-600 mt-1.5 flex items-center gap-1">
                      <CheckCircle size={11} /> This milestone is completed — choose a different one or create a standalone sprint.
                    </p>
                  )}
                </div>
              )}
              ...
              {form.selectedMilestoneTitle && (() => {
                const normalized = form.selectedMilestoneTitle.trim().toLowerCase();
                const ms = milestones.find(m => (m.title || "").trim().toLowerCase() === normalized);
                if (!ms) return null;
                ...
                const allocationMembers = projectAssignments.filter(
                  a => (a.task_name || "").trim().toLowerCase() === normalized
                );
                const totalAssigned =
                ...
                return (
                  <div className={`rounded-xl border px-4 py-3 ${hasTeam ? "bg-violet-50 border-violet-200" : "bg-amber-50 border-amber-200"}`}>
                    {ms.deliverable && (
                      <p className="text-xs text-slate-600 mb-2 italic">{ms.deliverable}</p>
                    )}
                    {hasTeam ? (
                      <p className="text-xs font-semibold text-emerald-700 flex items-center gap-1.5">
                        <Users size={12} /> {totalAssigned} member{totalAssigned !== 1 ? "s" : ""} assigned to this milestone
                      </p>
                    ) : (
                      <div className="flex items-start gap-2">
                        <AlertCircle size={13} className="text-amber-600 flex-shrink-0 mt-0.5" />
                        <div>
                          <p className="text-xs font-semibold text-amber-700">No team assigned yet</p>
                          <p className="text-[11px] text-amber-600 mt-0.5">
                            Assign team members via the Team Allocation page before using AI Populate.
                          </p>
                        </div>
                      </div>
                    )}
