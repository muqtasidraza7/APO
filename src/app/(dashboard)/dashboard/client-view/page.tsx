...
export default async function ClientViewPage() {
  ...
  const workspace = membership.workspace as unknown as { id: string; name: string };
  ...
  interface MilestoneRow { title: string; week: number; projectId: string; projectName: string; status: string }
  const allMilestones: MilestoneRow[] = [];
  projects.forEach((p) => {
    (p.ai_data?.milestones || []).forEach((m) => {
      allMilestones.push({ title: m.title, week: m.week, projectId: p.id, projectName: p.name, status: m.status || "pending" });
    });
  });
  ...
  const activeProjects  = projects.filter((p) => p.status === "active" || (!p.status && p.ai_status === "completed")).length;

  // Upcoming milestones grouped by week (max 8 weeks, 20 milestones)
  const upcoming = allMilestones.filter((m) => m.status !== "completed").slice(0, 20);
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-0.5 font-medium">Your project portfolio overview</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        >
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest text-violet-200 block mb-4">Projects</span>
          <div className="text-5xl font-black leading-none mb-3">{projects.length}</div>
        ...
        <div className="rounded-2xl p-6 bg-white border border-[#E8ECF4] shadow-sm">
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 block mb-4">Milestones</span>
          <div className="text-5xl font-black text-slate-900 leading-none mb-3">{doneMilestones}</div>
        ...
        {/* Team */}
        <div className="rounded-2xl p-6 bg-white border border-[#E8ECF4] shadow-sm">
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 block mb-4">Team</span>
          <div className="text-5xl font-black text-slate-900 leading-none mb-3">{headcount}</div>
          <div className="text-xs text-slate-400 font-medium">people on your projects</div>
        </div>
      </div>
        ...
        {projects.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {projects.map((p) => {
              ...
              const pDone  = pMilestones.filter((m) => m.status === "completed").length;
              const pTotal = pMilestones.length;
              const pPct   = pTotal > 0 ? Math.round((pDone / pTotal) * 100) : 0;

              const rawStatus = (p.status ?? (p.ai_status === "completed" ? "active" : "pending")) as StatusKey;
              const cfg = STATUS_CFG[rawStatus] ?? STATUS_CFG.pending;

              return (
                <div key={p.id} className="bg-white rounded-2xl p-5 border border-[#E8ECF4] shadow-sm hover:shadow-md hover:border-indigo-100 transition-all">
                  <div className="flex items-start justify-between gap-2 mb-4">
                    ...
                    <span className={`flex items-center gap-1.5 text-[10px] font-bold px-2 py-0.5 rounded-full border ${cfg.badge}`}>
                  ...
                  {pTotal > 0 ? (
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-[11px] text-slate-500 font-medium">{pDone}/{pTotal} milestones</span>
                        <span className="text-[11px] font-bold text-indigo-600">{pPct}%</span>
                      </div>
                    ...
                    <p className="text-[11px] text-slate-300 italic">No milestones planned yet</p>
                  )}
                </div>
              );
            })}
                    ...
                    {milestones.map((m, i) => (
                      <div key={i} className="flex items-start gap-2.5">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-slate-400 mt-0.5">{m.projectName}</p>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-[#E8ECF4] shadow-sm">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">{headcount} people working on your projects</p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-right">
              <div className="text-2xl font-black text-slate-900 leading-none">{headcount}</div>
              <div className="text-[10px] text-slate-400 font-semibold uppercase tracking-wide">members</div>
            </div>
          </div>
        </div>
      </div>
